...
class RKLLM:
    def __init__(self, model_path, platform):
        ...
        p.max_context_len, p.max_new_tokens = 4096, 4096
...
def prompt_from_messages(messages):
    ...
    for m in messages:
        ...
        if role == "assistant" and m.get("tool_calls"):
            value = json.dumps(m["tool_calls"], ensure_ascii=False)
        elif role == "tool":
            value = json.dumps({"tool_call_id": m.get("tool_call_id"), "content": text_content(m.get("content", ""))}, ensure_ascii=False)
        else:
            value = text_content(m.get("content", ""))
        out.append(f"{role}: {value}")
...
def make_sampling(data):
    ...
    p.top_k = int(data.get("top_k", 1)); p.top_p = float(data.get("top_p", .9))
    p.temperature = float(data.get("temperature", .8)); p.repeat_penalty = float(data.get("repeat_penalty", 1.1))
    p.frequency_penalty = float(data.get("frequency_penalty", 0)); p.presence_penalty = float(data.get("presence_penalty", 0))
    return p
...
def chat_completions():
    ...
    usage = {"prompt_tokens": len(prompt_from_messages(messages).split()), "completion_tokens": len(raw.split())}
    ...
    def stream_response():
        ...
        if calls:
            for i, call in enumerate(calls):
                delta = {"tool_calls": [{"index": i, "id": call["id"], "type": "function", "function": {"name": call["function"]["name"], "arguments": ""}}]}
                yield sse({**base, "choices": [{"index": 0, "delta": delta, "logprobs": None, "finish_reason": None}]})
                yield sse({**base, "choices": [{"index": 0, "delta": {"tool_calls": [{"index": i, "function": {"arguments": call["function"]["arguments"]}}]}, "logprobs": None, "finish_reason": None}]})
        else:
...
if __name__ == '__main__':
    ...
    ap.add_argument('--rkllm_model_path', required=True); ap.add_argument('--target_platform', required=True)
    args = ap.parse_args()
    ...
    def shutdown(signum, frame):
        rkllm_model.release(); raise SystemExit(0)
